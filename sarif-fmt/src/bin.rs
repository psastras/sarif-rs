...
fn main() -> Result<()> {
  ...
  match matches.value_of("message format").unwrap_or("pretty") {
    "plain" => {
      github::enable_plain_problem_matcher();
      let result = to_writer_plain(&sarif);
      github::disable_plain_problem_matcher();
      result
    }
    _ => {
      github::enable_pretty_problem_matcher();
      let result = to_writer_pretty(&sarif);
      github::disable_pretty_problem_matcher();
      result
    }
  }
}
